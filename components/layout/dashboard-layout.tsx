...
import { company, dashboardNav, futureModules } from "@/lib/constants/app";
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <nav className="flex-1 overflow-y-auto px-3 py-5">
            <p className="mb-2 px-2 text-[10px] font-semibold uppercase tracking-[0.15em] text-muted-foreground/50">Navigation</p>
            <div className="space-y-0.5">
              {dashboardNav.map((item) => {
                ...
                return (
                  ...
                  >
                    <item.icon className={cn("h-4 w-4 shrink-0", active ? "text-gold" : "text-muted-foreground/60")} />
                    {item.label}
                  </Link>
                );
              })}
          ...
          <div className="border-t border-border/50 px-4 py-4">
            <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-muted-foreground/50">Modules futurs</p>
            <div className="mt-3 space-y-2">
              {futureModules.map((module) => (
                <div key={module.title} className="flex items-center justify-between gap-2 rounded-sm bg-accent/5 px-2.5 py-1.5">
                  <span className="text-xs text-muted-foreground/60">{module.title}</span>
